...
public abstract class Task {
    ...
    private boolean isRunAsynchronous;
    ...
    public Task(String name) {
        this(name, android.os.Process.THREAD_PRIORITY_DEFAULT, false, 0);
    }

    public Task(String name, boolean asynchronous) {
        this(name, android.os.Process.THREAD_PRIORITY_DEFAULT, asynchronous, 0);
    }

    public Task(String name, int sequence) {
        this(name, android.os.Process.THREAD_PRIORITY_DEFAULT, false, sequence);
    }

    public Task(String name, boolean asynchronous, int sequence) {
        this(name, android.os.Process.THREAD_PRIORITY_DEFAULT, asynchronous, sequence);
    }
     ...
     * @param asynchronous   是否是异步任务（网络请求，rxjava）
     */
    public Task(String name, int threadPriority, boolean asynchronous, int currTaskSequence) {
        mName = name;
        mThreadPriority = threadPriority;
        this.isRunAsynchronous = asynchronous;
        mCurrTaskSequence = currTaskSequence;
    }
     ...
     */
    public Task(String name, boolean isInUiThread, boolean asynchronous, int currTaskSequence) {
        mName = name;
            ...
            if (!isRunAsynchronous) {
                ...
                mInternalRunnable = new Runnable() {
                    ...
                    public void run() {
                        ...
                        switchState(STATE_RUNNING);
                        Task.this.runAsynchronous(new OnTaskAnsyListener() {
                            @Override
                            public void onTaskFinish(String taskName, String taskStatus) {
                                switchState(STATE_FINISHED);

                                long finishTime = System.currentTimeMillis();
                                recordTime((finishTime - startTime));

                                notifyFinished(taskStatus);
                                recycle();
                            }
                        });
                    }
     ...
     * {@link OnTaskFinishListener#onTaskFinish(String, int, String)}}。</p>
    ...
    public abstract void runAsynchronous(OnTaskAnsyListener listener);
     ...
     */
    /*package*/ void notifyFinished(String taskStatus) {
        ...
        if (!mTaskFinishListeners.isEmpty()) {
            for (OnTaskFinishListener listener : mTaskFinishListeners) {
                listener.onTaskFinish(mName, mCurrTaskSequence, taskStatus);
            }
     ...
     * 执行自己的任务。<br>
     * 该函数由紧前{@code Task}的{@link #notifyFinished(String)}来调用。
     */
    /*package*/
    ...
    public interface OnTaskFinishListener {
         ...
         * @param currTaskSequence 当前结束的{@code Task}序号
         * @param taskStatus       当前结束的{@code Task}状态
         */
        void onTaskFinish(String taskName, int currTaskSequence, String taskStatus);
    }
    ...
    public interface OnTaskAnsyListener {
        /**
         * @param taskName   当前结束的{@code Task}名称
         * @param taskStatus 当前结束的{@code Task}状态，异步任务比如网络请求等一般有成功失败等状态，但都归结于任务结束
         */
        void onTaskFinish(String taskName, String taskStatus);
    }
